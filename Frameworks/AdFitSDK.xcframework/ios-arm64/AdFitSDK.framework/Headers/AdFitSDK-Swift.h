...
@optional
/// 광고 뷰 전체 영역에 대해 클릭 동작하도록 할지 여부를 리턴하도록 구현하세요.<br>
/// true를 리턴하도록 구현한 경우, 뷰 영역 내의 어느 지점을 클릭하더라도 광고주 페이지가 노출됩니다.<br>
/// false를 리턴하도록 구현한 경우, 뷰 내부의 개별 요소들을 클릭하는 경우에만 광고주 페이지가 노출됩니다.<br>
/// 이 함수를 구현하지 않은 경우, false를 리턴하도록 구현한 것과 동일하게 동작합니다.
/// :nodoc:
- (BOOL)isContainerClickable SWIFT_WARN_UNUSED_RESULT;
...
SWIFT_CLASS("_TtC8AdFitSDK18SuperboardBaseView")
@interface SuperboardBaseView : UIView
- (nonnull instancetype)initWithFrame:(CGRect)frame OBJC_DESIGNATED_INITIALIZER;
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)coder OBJC_DESIGNATED_INITIALIZER;
@end

@protocol AdFitFlexibleViewDelegate;
@protocol AdFitFlexibleViewVideoDelegate;
SWIFT_CLASS("_TtC8AdFitSDK17AdFitFlexibleView")
@interface AdFitFlexibleView : SuperboardBaseView
@property (nonatomic, readonly, copy) NSString * _Nonnull adUnitId;
@property (nonatomic) CGFloat desiredMediaWidth;
@property (nonatomic, weak) id <AdFitFlexibleViewDelegate> _Nullable delegate;
@property (nonatomic, weak) id <AdFitFlexibleViewVideoDelegate> _Nullable videoDelegate;
@property (nonatomic, readonly) CGFloat adRatio;
/// 광고가 노출될 뷰를 초기화 합니다.
/// \param adUnitId 광고 단위 ID
///
///
/// returns:
/// <code>AdFitFlexibleView</code> 객체
...
SWIFT_PROTOCOL("_TtP8AdFitSDK25AdFitFlexibleViewDelegate_")
@protocol AdFitFlexibleViewDelegate <NSObject>
@optional
/// 배너 광고가 성공적으로 로드 되었을 때 호출됩니다.
/// \param adView 광고가 로드된 배너 광고 뷰 객체
///
- (void)adViewDidReceiveAd:(AdFitFlexibleView * _Nonnull)adView;
/// 배너 광고 로드에 실패한 경우 호출됩니다.
/// \param adView 광고 로드에 실패한 배너 광고 뷰 객체
///
/// \param error 광고 로드 실패의 원인이 된 에러 객체
///
- (void)adViewDidFailToReceiveAd:(AdFitFlexibleView * _Nonnull)adView error:(NSError * _Nonnull)error;
/// 배너 광고에서 사용자 클릭이 발생하였을 때 호출됩니다.<br>
/// 클릭에 의해 광고주 페이지가 전체 화면으로 노출되므로, 화면 전환시 필요한 동작이 있다면 이 메서드에 구현하세요.
/// \param adView 클릭 이벤트가 발생한 배너 광고 뷰 객체
///
- (void)adViewDidClickAd:(AdFitFlexibleView * _Nonnull)adView;
@end

SWIFT_PROTOCOL("_TtP8AdFitSDK30AdFitFlexibleViewVideoDelegate_")
@protocol AdFitFlexibleViewVideoDelegate
@optional
/// 비디오 재생이 시작되었을 때 호출됩니다.
- (void)flexibleViewVideoDidPlay:(AdFitFlexibleView * _Nonnull)adView;
/// 비디오 재생이 일시 중지되었을 때 호출됩니다.
- (void)flexibleViewVideoDidPause:(AdFitFlexibleView * _Nonnull)adView;
/// 비디오 재생이 완료되었을 때 호출됩니다.
- (void)flexibleViewVideoDidComplete:(AdFitFlexibleView * _Nonnull)adView;
/// 비디오 음소거가 활성화되었을 때 호출됩니다.
- (void)flexibleViewVideoDidMute:(AdFitFlexibleView * _Nonnull)adView;
/// 비디오 음소거가 비활성화되었을 때 호출됩니다.
- (void)flexibleViewVideoDidUnmute:(AdFitFlexibleView * _Nonnull)adView;
/// 비디오 재생 중 오류가 발생했을 때 호출됩니다.
- (void)flexibleViewVideoDidFailed:(AdFitFlexibleView * _Nonnull)adView error:(NSError * _Nonnull)error;
/// 비디오 버퍼링 상태가 변경되었을 때 호출됩니다.
- (void)flexibleViewVideoDidChangeBuffering:(AdFitFlexibleView * _Nonnull)adView buffering:(BOOL)buffering;
/// 비디오 재생 시간이 변경되었을 때 호출됩니다.
...
/// 이미지 컬렉션 (메인이미지 + 카탈로그)
  AdFitMediaTypeImageCollection = 8,
/// 비디오 컬렉션 (비디오 + 카탈로그)
  AdFitMediaTypeVideoCollection = 9,
};
...
/// 현재 표시중인 미디어의 타입이 비디오 일때 컨트롤뷰의 사용여부를 선택 할 수 있습니다.
@property (nonatomic) BOOL useVideoControlView;
...
/// \param mediaView 음소거 해제된 비디오를 표시하고 있는 미디어 뷰 객체
///
/// \param current 재생중 플레이 시간
///
/// \param total 총 플레이 시간
///
- (void)mediaViewVideoDidDurationChange:(AdFitMediaView * _Nonnull)mediaView current:(NSTimeInterval)current total:(NSTimeInterval)total;
/// 미디어 뷰에 표시된 비디오의 재생이 실패하는 시점에 호출됩니다.
/// \param mediaView 음소거 해제된 비디오를 표시하고 있는 미디어 뷰 객체
///
/// \param error 비디오 재생에 실패한 에러 상황
///
- (void)mediaViewVideoDidFailed:(AdFitMediaView * _Nonnull)mediaView error:(NSError * _Nonnull)error;
/// 미디오 뷰에 표시된 비디오의 재생상태가 버퍼링으로 변화하는 시점에 호출됩니다.
/// \param mediaView 음소거 해제된 비디오를 표시하고 있는 미디어 뷰 객체
///
- (void)mediaViewVideoDidChangeBuffering:(AdFitMediaView * _Nonnull)mediaView buffering:(BOOL)buffering;
@end
...
- (nonnull instancetype)initWithClientId:(NSString * _Nonnull)clientId count:(NSInteger)count userObject:(UserObject * _Nullable)userObject contentObject:(ContentObject * _Nullable)contentObject OBJC_DESIGNATED_INITIALIZER;
/// 네이티브 광고를 요청합니다.
/// note:
...
/// 모션보드의 플레이 유형.
typedef SWIFT_ENUM(NSInteger, AdMotionPlayType, open) {
/// 영상 자동 재생 & 애니메이션 모션
  AdMotionPlayTypeAll = 0,
/// 애니메이션 모션
  AdMotionPlayTypeOnlyAnimation = 1,
/// 스틸컷(정지이미지)
  AdMotionPlayTypeStillCut = 2,
};
...
SWIFT_CLASS("_TtC8AdFitSDK11FrameLayout")
@interface FrameLayout : VirtualLayout
- (nonnull instancetype)initWithFrame:(CGRect)frame OBJC_DESIGNATED_INITIALIZER;
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)coder OBJC_DESIGNATED_INITIALIZER;
@end

SWIFT_CLASS("_TtC8AdFitSDK12LinearLayout")
@interface LinearLayout : VirtualLayout
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)aDecoder OBJC_DESIGNATED_INITIALIZER;
- (void)willRemoveSubview:(UIView * _Nonnull)subview;
- (nonnull instancetype)initWithFrame:(CGRect)frame SWIFT_UNAVAILABLE;
@end
...
@interface NasAdLoader : NSObject
@property (nonatomic, weak) id <NasAdLoaderDelegate> _Nullable delegate;
@property (nonatomic) CGFloat desiredMediaWidth;
- (nonnull instancetype)initWithAdUnitId:(NSString * _Nonnull)adUnitId OBJC_DESIGNATED_INITIALIZER;
- (void)loadAd;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
...
@class NasAdViewModel;
SWIFT_PROTOCOL("_TtP8AdFitSDK19NasAdLoaderDelegate_")
@protocol NasAdLoaderDelegate <NSObject>
@optional
- (void)didReceivedAd:(NasAdViewModel * _Nonnull)viewModel;
- (void)didFailToReceiveAdWithError:(NSError * _Nonnull)error;
@end

SWIFT_CLASS("_TtC8AdFitSDK9NasAdView")
@interface NasAdView : SuperboardBaseView
- (void)addSubview:(UIView * _Nonnull)view;
- (nonnull instancetype)initWithFrame:(CGRect)frame SWIFT_UNAVAILABLE;
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)coder SWIFT_UNAVAILABLE;
@end

SWIFT_PROTOCOL("_TtP8AdFitSDK17NasAdViewDelegate_")
@protocol NasAdViewDelegate <NSObject>
@optional
- (void)adViewDidRendered:(NasAdView * _Nonnull)adView;
- (void)adViewDidFailToRender:(NasAdView * _Nonnull)adView error:(NSError * _Nonnull)error;
- (void)adViewDidClicked:(NasAdView * _Nonnull)adView;
@end

SWIFT_CLASS("_TtC8AdFitSDK14NasAdViewModel")
@interface NasAdViewModel : NSObject
@property (nonatomic, readonly) CGFloat adAspectRatio;
@property (nonatomic, readonly) CGFloat maxHeight;
@property (nonatomic, weak) UIViewController * _Nullable rootViewController;
/// 사용자가 View를 이탈했다가 다시 바인딩 합니다. (Life cyle - ViewWillAppear)
/// note:
/// 이 메서드는 광고가 로드 된 이후에 호출되어야 합니다. 광고 로드 이전에 호출되면 아무런 동작도 하지 않습니다.
/// note:
/// 이 메서드는 반드시 main queue 에서 사용해야 합니다.
- (void)bind;
/// 사용자가 View를 이탈하여 바인딩을 해제합니다. (Life cycle - ViewWillDisappear)
/// note:
/// 이 메서드는 광고가 로드 된 이후에 호출되어야 합니다. 광고 로드 이전에 호출되면 아무런 동작도 하지 않습니다.
/// note:
/// 이 메서드는 반드시 main queue 에서 사용해야 합니다.
...
SWIFT_CLASS("_TtC8AdFitSDK16NasResizableView")
@interface NasResizableView : UIView
- (void)willMoveToSuperview:(UIView * _Nullable)newSuperview;
- (void)observeValueForKeyPath:(NSString * _Nullable)keyPath ofObject:(id _Nullable)object change:(NSDictionary<NSKeyValueChangeKey, id> * _Nullable)change context:(void * _Nullable)context;
- (void)layoutSubviews;
- (nonnull instancetype)initWithFrame:(CGRect)frame OBJC_DESIGNATED_INITIALIZER;
